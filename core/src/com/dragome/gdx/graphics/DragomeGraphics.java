...
/** Default implementation of {@link Graphics} for Dragome applications. Wraps around a HTML canvas and WebGL. Does not
 * support GL30. Allows to go to fullscreen mode only if chosen display mode matches current screen size. Allows to
 * change cursors and current page title. Always reports only one {@link Monitor} and {@link DisplayMode}. Does not
 * support vSync settings.
 *
 * @author MJ */
public class DragomeGraphics implements Graphics {
    ...
    private final DisplayMode displayMode = new DragomeDisplayMode(getScreenWidth(), getScreenHeight(), REFRESH_RATE,
            BPP);
    ...
    public GL30 getGL30() {
        application.error(DragomeApplication.LOGGING_TAG,
                "Note: DragomeGraphics#getGL30 is not supported on Dragome backend.");
        return null;
    }
    ...
    /** Will post an event which will resize the game during the next render call. Width and height have to match
    ...
    /** Attempts to lock orientation according to application's configuration object. Should be executed after entering
     * fullscreen mode. */
    ...
    /** Enum values from http://www.w3.org/TR/screen-orientation. Filtered based on what the browsers actually
     * support. */
