...
/** Default {@link Renderer} implementation for Dragome applications. Manages main loop. Note that it always reports 0
 * for every {@link #getFrameId()} and {@link #getFramesPerSecond()}; if you want to keep track of rendering meta-data,
 * use {@link DebugDragomeRenderer}.
 *
 * @author MJ */
public class DragomeRenderer implements Renderer {
    ...
    /** @param runnable will be executed as the animation frame. If it is the main loop runnable, it should recursively
     *            invoke this method passing itself as the parameter. */
    ...
    protected void cancelAnimationFrame() {
        ScriptHelper.evalNoResult("if(typeof this._frameId==='number'){window.cancelAnimationFrame(this._frameId);}",
                this);
    }
